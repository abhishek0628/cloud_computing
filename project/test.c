...
#include <stdlib.h>
#include <assert.h>
#include <string.h>   /* for strcmp, getenv */
...
/* ---- LIP_i controls (set via environment variables) ----
   LIP_ON=1 enables LIP_i for the LLC ("ul2" or "dl2"); LIP_ON=0 = baseline LRU.
   LIP_I=<int> chooses the insertion position i (0 = LRU, larger = closer to MRU).
   Defaults: OFF, i=15 (i.e., insert at MRU == normal LRU).
*/
static int g_lip_enable = 0;     /* 0=off, 1=on */
static int g_lip_i      = 15;    /* insertion position; 15 ~ MRU (baseline) */
static int g_lip_init   = 0;     /* init guard */

static void lip_init_once(void)
{
  if (g_lip_init) return;
  {
    char *on = getenv("LIP_ON");
    char *iv = getenv("LIP_I");
    if (on) g_lip_enable = atoi(on) ? 1 : 0;
    if (iv) g_lip_i = atoi(iv);
    if (g_lip_i < 0) g_lip_i = 0;
  }
  g_lip_init = 1;
}
...
/* ---- LIP_i helper: insert block at position i from LRU tail ----
   0 = exactly at tail (LRU), 1 = just before tail, ..., big = near head (MRU).
   If pos_from_tail exceeds list length, BLK goes to the head (MRU).
*/
static void insert_from_tail(struct cache_set_t *set,
                             struct cache_blk_t *blk,
                             int pos_from_tail)
{
  /* unlink BLK from its current position (like update_way_list does) */
  if (!blk->way_prev && !blk->way_next) {
    /* single-entry list: nothing to do */
    return;
  } else if (!blk->way_prev) {
    /* head */
    set->way_head = blk->way_next;
  ...
  /* walk up from tail towards head */
  struct cache_blk_t *cur = set->way_tail;
  int steps = pos_from_tail;
  while (steps-- > 0 && cur->way_prev)
    cur = cur->way_prev;

  /* if exceeded head, put at head (MRU) */
  if (!cur->way_prev && steps > 0) {
    blk->way_next = set->way_head;
    blk->way_prev = NULL;
    set->way_head->way_prev = blk;
    set->way_head = blk;
  ...
  /* insert BEFORE 'cur' */
  blk->way_next = cur;
  blk->way_prev = cur->way_prev;
  if (cur->way_prev)
    cur->way_prev->way_next = blk;
  else
    set->way_head = blk; /* inserted at head */
  cur->way_prev = blk;
}
	     ...
	     unsigned int hit_latency)	/* latency in cycles for a hit */
{
  ...
  {
    int bindex_local = 0;
    for (bindex=bindex_local,i=0; i<nsets; i++)
      {
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  case FIFO:
    ...
    /* ---- LIP_i insertion for LLC (ul2 or dl2) ---- */
    lip_init_once();
    if (g_lip_enable && cp->name &&
        (strcmp(cp->name, "ul2") == 0 || strcmp(cp->name, "dl2") == 0))
    {
      /* Insert new block at position i-from-tail (0=LRU ... high=MRU) */
      insert_from_tail(&cp->sets[set], repl, g_lip_i);
    }
    else
    {
      /* Baseline: insert at MRU (head) like normal LRU/FIFO */
