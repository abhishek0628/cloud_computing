...
MODEL_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "experiments", "encoder_decoder.pth"))
...
def evaluate(num_tests=30):
    ...
    for _ in range(num_tests):
        ...
        vnf_tensor = torch.tensor([[v['cpu'], v['ram'], v['bw'], v['energy'], v['duration']] for v in sfc], dtype=torch.float32)
        ...
        for k in range(len(encoded)):
            dec_out = decoder.decoder(encoded.unsqueeze(0)[:, :k+1, :], decoder.node_fc(path_features).unsqueeze(0))[:, -1, :]
    ...
    # Save data
    SAVE_DIR = os.path.join(os.path.abspath(os.path.join(BASE, "..")), "experiments")
    os.makedirs(SAVE_DIR, exist_ok=True)
    np.save(os.path.join(SAVE_DIR, "rl_scores.npy"), np.array(rl_scores))
    np.save(os.path.join(SAVE_DIR, "rnd_scores.npy"), np.array(rnd_scores))
    ...
    plt.grid(True)
    plt.savefig(os.path.join(SAVE_DIR, "rl_vs_random.png"))
    print(f"RL Avg Variance: {np.mean(rl_scores):.4f}")
    print(f"Random Avg Variance: {np.mean(rnd_scores):.4f}")
    print("Saved plot: rl_vs_random.png")
